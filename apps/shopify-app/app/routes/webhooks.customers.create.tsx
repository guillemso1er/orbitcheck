import { shopifyCustomersCreateWebhook } from "@orbitcheck/contracts";
...
export const action = async ({ request }: ActionFunctionArgs) => {
  ...
  try {
    ...
    await shopifyCustomersCreateWebhook<true>({
    ...
    if (msg === 'fetch failed' || msg.includes('ECONNREFUSED')) {
      console.warn("Failed to forward customers/create webhook to OrbitCheck API: API appears to be down.");
    } else {
      console.error("Failed to forward customers/create webhook to OrbitCheck API", error);
    }
  }
