...
/**
 * BullMQ processor for address fix fulfillment hold workflow
 * Polls for fulfillment orders and applies holds
 */
export async function addressFixProcessor(job: Job<AddressFixJobData>): Promise<void> {
    const { shopDomain, orderGid, sessionId, pool, logger } = job.data;
