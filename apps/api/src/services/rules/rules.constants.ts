...
export function getBuiltInRules() {
    const rules = [
        { id: 'email_format', name: 'Email Format Validation', description: 'Validates the basic format of email addresses using RFC standards.', category: 'email', enabled: false, condition: '(email && email.valid === false) || (emailString && !/^[^\\s@]+@[^\\s@]+\\.[^\\s@]+$/.test(emailString))', action: 'hold', priority: 10 },
        { id: 'email_mx', name: 'Email MX Record Check', description: 'Verifies that the domain has valid MX records for email delivery.', category: 'email', enabled: false, condition: 'email && email.mx_records === false', action: 'hold', priority: 8 },
        { id: 'email_disposable', name: 'Disposable Email Detection', description: 'Detects and flags temporary or disposable email services.', category: 'email', enabled: false, condition: 'email && email.disposable === true', action: 'block', priority: 15 },
        { id: 'po_box_detection', name: 'PO Box Detection', description: 'Identifies and flags addresses using PO Box or similar mail services.', category: 'address', enabled: false, condition: 'address && address.po_box === true', action: 'block', priority: 12 },
        ...
        { id: 'order_dedupe', name: 'Order Deduplication', description: 'Checks for potential duplicate orders based on customer and address data.', category: 'order', enabled: false, condition: 'email && email.normalized && phoneString && address && name && session_id && transaction_amount', action: 'hold', priority: 14 },
        ...
        { id: 'address_validation', name: 'Address Validation', description: 'Validates and normalizes physical addresses for accuracy and deliverability.', category: 'address', enabled: false, condition: 'address && !address.valid', action: 'hold', priority: 9 },
        { id: 'address_geocode', name: 'Address Geocoding Validation', description: 'Normalizes and validates physical addresses against geographic data.', category: 'address', enabled: false, condition: 'address && address.valid === false', action: 'hold', priority: 8 },
